...
export default function Login() {
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook' | 'apple') => {
    ...
    try {
      ...
      setError(`Para usar ${provider === 'google' ? 'Google' : provider === 'facebook' ? 'Facebook' : 'Apple'}, activa el provider en Supabase Dashboard → Authentication → Providers`);
      setCargandoProvider('');
    }
  };
    ...
    <main className="min-h-screen bg-white flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button onClick={() => handleSocialLogin('facebook')} disabled={!!cargandoProvider}
            className="w-full py-3.5 border-2 border-gray-200 rounded-2xl font-semibold text-gray-700 flex items-center justify-center gap-3 hover:border-gray-300 hover:bg-gray-50 transition disabled:opacity-50">
            {cargandoProvider === 'facebook' ? <div className="w-5 h-5 border-2 border-gray-400 border-t-transparent rounded-full animate-spin"/> : (
          ...
          <button onClick={() => handleSocialLogin('apple')} disabled={!!cargandoProvider}
            className="w-full py-3.5 bg-black rounded-2xl font-semibold text-white flex items-center justify-center gap-3 hover:bg-gray-900 transition disabled:opacity-50">
            {cargandoProvider === 'apple' ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"/> : (
