...
export default function HomeViajero() {
  ...
  return (
    <main className="min-h-screen bg-sky-50 pb-32">
      ...
      {/* Header Viajero — cielo/teal, aventura */}
      <div className="bg-gradient-to-br from-sky-500 via-teal-500 to-emerald-600 px-6 pt-12 pb-8 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-8 bg-white/5" style={{borderRadius: '50% 50% 0 0 / 100% 100% 0 0'}}/>
          ...
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <h1 className="text-2xl font-extrabold text-white">
                {usuario?.nombre?.split(' ')[0] || 'Viajero'} 🌍
              </h1>
            </div>
          ...
          <div className="bg-white/15 backdrop-blur rounded-2xl p-4 mb-3 border border-white/25">
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={() => setEditandoCiudad(!editandoCiudad)}
                className="text-xs bg-white/20 px-2 py-1 rounded-full font-semibold text-white hover:bg-white/30 transition">
            ...
            {editandoCiudad ? (
              <div className="flex gap-2 mt-2">
                ...
                <button onClick={guardarCiudad} disabled={guardandoCiudad}
                  className="px-3 py-2 bg-white text-teal-600 rounded-xl font-bold text-sm disabled:opacity-50">✓</button>
              </div>
            ) : (
          ...
          {usuario?.ciudades_visitadas?.length > 0 && (
            <div className="flex gap-2 overflow-x-auto pb-1 mb-3">
              {usuario.ciudades_visitadas.map((c: string, i: number) => (
                <span key={i} className="flex-shrink-0 text-xs bg-white/20 text-white font-semibold px-3 py-1.5 rounded-full border border-white/25">
                  📍 {c}
                </span>
              ))}
            </div>
            ...
            {trabajosFiltrados.map((trabajo) => {
              ...
              return (
                <a href={`/trabajo?id=${trabajo.id}`} key={trabajo.id}
                  className="bg-white rounded-2xl p-4 shadow-sm border border-sky-100 active:scale-95 transition block">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="text-right">
                          ...
                          <span className={`mt-1 text-xs font-bold px-3 py-1 rounded-full inline-block ${
                            yaAplico ? 'bg-green-100 text-green-600' : 'bg-gradient-to-r from-sky-500 to-teal-500 text-white'
                          }`}>
